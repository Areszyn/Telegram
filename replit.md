...
- **Managed Bots (Bot API 9.6)**: Create and manage bots on behalf of users. Both admin and user-facing.
    ...
    - **User**: "My Bots" page (`/my-bots`) in user navigation. Users create bots via `t.me/lifegrambot/newbot` with mandatory name + username fields (username must end with "bot"). Features: token reveal/copy/rotate, activate/deactivate, auto-reply, message forwarding, bot description sync. Owner-scoped API routes: `GET /my-bots`, `POST /my-bots/create-link`, `POST /my-bots/configure`, `POST /my-bots/setup-webhook`, `POST /my-bots/remove-webhook`, `GET /my-bots/:id/info`, `POST /my-bots/get-token`, `POST /my-bots/rotate-token`.
