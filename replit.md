...
- **Managed Bots (Bot API 9.6)**: Create and manage bots on behalf of users. Webhook handler for `managed_bot` update type auto-upserts bots to `managed_bots` D1 table. Admin commands: `/managed list|create|token|rotate`. Admin API routes in `bot-admin.ts`. ManagedBots UI component in admin Bot Tools page. Create link uses `t.me/newbot/lifegrambot`. Token get/rotate via Telegram `getManagedBotToken`/`replaceManagedBotToken` APIs with numeric validation.
- **AI Chat Hub (BYOK)**:
