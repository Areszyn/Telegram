...
notices.get("/app-notice", async (c) => {
  const row = await c.env.DB.prepare(
    "SELECT id, title, message, type, button_text FROM app_notices WHERE active = 1 ORDER BY id DESC LIMIT 1"
  ).first<{ id: number; title: string; message: string; type: string; button_text: string | null }>();
...
notices.get("/admin/notices", requireAdmin(), async (c) => {
  const { results } = await c.env.DB.prepare(
    "SELECT id, title, message, type, button_text, active, created_at FROM app_notices ORDER BY id DESC LIMIT 50"
  ).all();
  return c.json({ notices: results ?? [] });
});

notices.post("/admin/notices", requireAdmin(), async (c) => {
  const { title, message, type, button_text } = await c.req.json<{ title?: string; message?: string; type?: string; button_text?: string }>();
  if (!message?.trim()) return c.json({ error: "Message is required" }, 400);
  await c.env.DB.prepare("UPDATE app_notices SET active = 0 WHERE active = 1").run();
  const res = await c.env.DB.prepare(
    "INSERT INTO app_notices (title, message, type, button_text, active) VALUES (?, ?, ?, ?, 1)"
  ).bind(title?.trim() || "", message.trim(), type?.trim() || "info", button_text?.trim() || null).run();
  return c.json({ ok: true, id: res.meta.last_row_id });
});

notices.put("/admin/notices/:id", requireAdmin(), async (c) => {
  const id = c.req.param("id");
  const { title, message, type, button_text, active } = await c.req.json<{ title?: string; message?: string; type?: string; button_text?: string; active?: boolean }>();
  const sets: string[] = [];
  const vals: unknown[] = [];
  ...
  if (button_text !== undefined) { sets.push("button_text = ?"); vals.push(button_text.trim() || null); }
  if (active !== undefined) {
    if (active) {
