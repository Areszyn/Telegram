...
async function streamFile(
  ...
  if (!payload) {
    res.status(410).type("text").send("Link expired or invalid.");
    return;
  }
...
</head><body><p>⏰ This link has expired</p><small>Video links are valid for 24 hours.</small></body></html>`);
    return;
  }
...
.progress-track{
  ...
  cursor:pointer;position:relative;margin-bottom:10px;transition:height .15s;
}
...
.play-bar{
  position:absolute;inset:0;background:#3b82f6;
  border-radius:2px;width:0;pointer-events:none;
}
.thumb{
...
.btn{
  ...
  padding:4px 5px;opacity:.8;font-size:18px;line-height:1;
...
/* — title bar — */
.titlebar{
  padding:8px 14px 6px;font-size:13px;color:#888;
...
a.dlbtn{
  color:#3b82f6;text-decoration:none;font-size:13px;
  padding:6px 16px;border:1px solid rgba(59,130,246,.4);
  border-radius:8px;transition:background .15s;
}
a.dlbtn:hover{background:rgba(59,130,246,.12)}

@media(max-width:600px){
  video{max-height:55svh}
  .titlebar{font-size:12px;padding:6px 10px 4px}
...
(function(){
  ...
  fullbtn.addEventListener('click', () => {
    if (document.fullscreenElement) {
      ...
      const el = v.webkitSupportsFullscreen ? v : document.querySelector('.wrap');
